  ...
  try {
    ...
    for (let i = 0; i < nikList.length; i += batchSize) {
      ...
      const query = `
        ...
        FROM petani_belum_tebus_region2a
...
// **ENDPOINT UTAMA YANG DIPERBAIKI: POST /api/region2a**
router.post('/', upload.single('file'), handleUploadError, async (req, res) => {
  console.log('✅ Received POST request to /api/region2a');
  ...
  try {
    ...
    res.json({
      ...
      stats: stats,
      download_url: `/api/region2a/download/${resultFilename}`,
      removed_data_url: `/api/region2a/download-removed/${removedFilename}` // URL baru untuk download data yang dihapus
    });
...
// **ENDPOINT DOWNLOAD: GET /api/region2a/download/:filename** (untuk hasil filter)
router.get('/download/:filename', async (req, res) => {
  try {
    const filename = req.params.filename;
    
    if (!filename.match(/^hasil_filter_[a-zA-Z0-9_-]+\.xlsx$/)) {
      return res.status(400).json({
        success: false,
...
// **ENDPOINT BARU: DOWNLOAD DATA YANG DI-REMOVE** 
