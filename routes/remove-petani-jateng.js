  ...
  try {
    ...
    for (let i = 0; i < nikList.length; i += batchSize) {
      ...
      const query = `
        ...
        FROM petani_belum_tebus_jateng
...
// **ENDPOINT UTAMA YANG DIPERBAIKI: POST /api/jateng**
router.post('/', upload.single('file'), handleUploadError, async (req, res) => {
  console.log('✅ Received POST request to /api/jateng');
  ...
  try {
    ...
    res.json({
      ...
      stats: stats,
      download_url: `/api/jateng/download/${resultFilename}`,
      removed_data_url: `/api/jateng/download-removed/${removedFilename}` // URL baru untuk download data yang dihapus
    });
...
// **ENDPOINT DOWNLOAD: GET /api/jateng/download/:filename** (untuk hasil filter)
